...
module.exports = {
	...
	async execute(interaction) {
        ...
        // VERIFICATION PROMPT
            ...
            let clickUser = await button.clicker.user.fetch();   // user = @MrMusicMan789 (OBJECT)
            ...
            message.channel.send(`clickUser = ${clickUser}\nclickUserTag = ${clickUserTag}\nclickUsername = ${clickUsername}\nclickUserId = ${clickUserId}`)

            // DATABASE RETRIEVING TICKET CATEGORY FOR CHANNEL CREATION
            try {
                console.log(`Retrieving TICKET_CAT_ID via findOne for guild ${button.guild.name} from the roles Verificiation embed.`)
                ...
                console.log(`TICKET_CAT_ID = ${ticketCategory}`)
            } catch (err) {
                console.log(err)
            }
