...
cron.schedule('00 39 15 * * *', async () => {
    ...
    if(dbBirthdayData) {
        ...
        let birthdayRole = guild.roles.cache.find(role => role.name.toLowerCase().startsWith(`birthday`))
        ...
        result.forEach( id => {
            ...
            guild.members.fetch(id).roles.add(birthdayRole)
        })
    }
}, {
    scheduled: true,
